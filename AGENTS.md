...
- ゲームは three.js 製ブラウザ FPS「FPS ARENA」（ビルド不要・ローカル同梱 three.js）。マップは DESERT / JUNGLE（追加予定あり）。ロビーの MAP カードで選択（背景も即切替）。モードは Survival（5ステージ・テーマ付き）と TDM（5v5・5分・キル数勝負）。TDM はロビーで LOCAL（AI）と ONLINE（ルーム）に分岐。オンラインパネルは接続前（部屋を建てる/コードで入る）⇔接続後（名簿・名前入力・開始ボタン）を `setOnlinePanelJoined` で切替。`.orow[hidden]` の CSS 上書きに注意（author スタイルが UA の hidden を打ち消す）
