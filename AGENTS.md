...
- 継続開発の本体はローカル `/Users/hayashi/work/projects/karakin`。Git 正本 remote は `origin` → `hayashiii-ghub/fps-game`。公開は Cloudflare Pages（`https://kimi-grok-fps.pages.dev/`、プロジェクト名 `kimi-grok-fps`）。旧 `hayashiii-ghub/karakin` は Archive 済み、`karakin-demo` も凍結。push は `origin`（fps-game）一本でよい
