...
- ゲームは three.js 製ブラウザ FPS「FPS ARENA」（ビルド不要・ローカル同梱 three.js）。マップは DESERT / JUNGLE（ともに Kimi K3 制作。いろいろなモデルに作らせて追加していく方針）。1マップ＝`maps/<id>.js` の1ファイルで、`shared/map-manifest.js` に ID・表示名・制作モデル・天候・限定武器を登録し、`world.js` の `registerMap()` へ build 関数を渡す。ロビーの MAP カードは `MAP_DEFS` から自動生成され（`renderMapCards`）、`MAP BY <model>` を表示するので index.html へ個別ボタンを足さない。モデルへ渡す制作ガイドは `MAP_AUTHORING.md`。モードは Survival（5ステージ・テーマ付き）と TDM（5v5・5分・キル数勝負）。TDM はロビーで LOCAL（AI）と ONLINE（ルーム）に分岐。オンラインパネルは接続前（部屋を建てる/コードで入る）⇔接続後（名簿・名前入力・開始ボタン）を `setOnlinePanelJoined` で切替。`.orow[hidden]` の CSS 上書きに注意（author スタイルが UA の hidden を打ち消す）
