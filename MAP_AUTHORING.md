...
- 街区は`streetTerrace(x, z, w, h, d, face, mats, shops)`で作る。間口4〜9mの細い棟に割れ、棟ごとに高さ・素材・業態が変わる。大きな箱を1つ置くと目線の高さでは平らな壁1枚にしかならず、表面を飾っても街に見えない
- 単独棟は`towerGlass` / `towerStone` / `midGrid` / `lowPanel` / `pilotis`を混ぜる。同じヘルパーばかり並べると使い回しに見える
- 通りに面した1階は`shopfront(x, z, w, d, face, kind, span)`で店舗にする。`kind`は`SHOP_KINDS`の21種（コンビニ3系統・飲食8・物販サービス8・`lobby`・`shutter`）。壁自体が固体なので移動コライダは増えない
- 業態は方角で固めず、全街区で同じ一覧から引いて混ぜる。看板は配色と帯の本数だけで業態を示し、社名・ロゴ・書体は入れない
- 窓のあるファサードは`facadeSolid()`で作る。`solid()`に直接ファサード素材を渡すとテクスチャ1枚が面全体へ引き伸ばされ、建物の大きさに関わらず同じ窓数になる（階高が破綻する）
- 街路壁の裏は`backAlley()`でバックヤードにする。表通りと対比させる面で、通路を残せば回り込み経路も潰さない
- 建物ごとのばらつきは`Math.random()`ではなく`siteRand(x, z, salt)`を使う。`|z|`基準なので、`zPair`で南北に置いても同じ結果になり青赤が不公平にならない
